            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('template_id') is-invalid @enderror"
                        name="template_id" value="{{ old('template_id') }}" placeholder="Enter template id">
                    @error('template_id')
                ...
                <div class="form-group">
                    ...
                    <textarea name="template" class="form-control @error('template') is-invalid @enderror" cols="30" rows="10"
                        placeholder="Enter template">{{ old('template') }}</textarea>
                    @error('template')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('has_param') is-invalid @enderror" name="has_param">
                        ...
                        <option value="1" {{ old('has_param') == '1' ? 'selected' : '' }}>Yes</option>
                        <option value="0" {{ old('has_param') == '0' ? 'selected' : '' }}>No</option>
                    </select>
                    @error('has_param')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
